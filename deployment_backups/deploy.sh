...
# Check that required regions are specified
if [ -z "$BGP_REGION_PRIMARY" ]; then
  echo "Error: BGP_REGION_PRIMARY must be specified in .env file"
  exit 1
fi
if [ -z "$BGP_REGION_SECONDARY" ]; then
  echo "Error: BGP_REGION_SECONDARY must be specified in .env file"
  exit 1
fi
if [ -z "$BGP_REGION_TERTIARY" ]; then
  echo "Error: BGP_REGION_TERTIARY must be specified in .env file"
  exit 1
fi

# Set quaternary region, defaulting to tertiary if not specified
BGP_REGION_QUATERNARY=${BGP_REGION_QUATERNARY:-${BGP_REGION_TERTIARY}}

# Create arrays for BGP regions
BGP_REGIONS=("$BGP_REGION_PRIMARY" "$BGP_REGION_SECONDARY" "$BGP_REGION_TERTIARY" "$BGP_REGION_QUATERNARY")

# For backward compatibility - will be deprecated in future versions
IPV4_REGION_PRIMARY=${IPV4_REGION_PRIMARY:-${BGP_REGION_PRIMARY}}
IPV4_REGION_SECONDARY=${IPV4_REGION_SECONDARY:-${BGP_REGION_SECONDARY}}
IPV4_REGION_TERTIARY=${IPV4_REGION_TERTIARY:-${BGP_REGION_TERTIARY}}
IPV4_REGIONS=("$IPV4_REGION_PRIMARY" "$IPV4_REGION_SECONDARY" "$IPV4_REGION_TERTIARY")
IPV6_REGION=${IPV6_REGION:-${BGP_REGION_QUATERNARY}} # For dual-stack, quaternary server handles IPv6 by default
...
deploy() {
  ...
  if [ "$force_fresh" = true ]; then
    ...
    case "${IP_STACK_MODE:-dual}" in
      ipv4)
        ...
        create_instance "${IPV4_REGIONS[0]}" "${IPV4_REGIONS[0]}-ipv4-bgp-primary-1c1g" "1" "false" || { echo "Failed to create primary instance"; exit 1; }
        create_instance "${IPV4_REGIONS[1]}" "${IPV4_REGIONS[1]}-ipv4-bgp-secondary-1c1g" "2" "false" || { echo "Failed to create secondary instance"; exit 1; }
        create_instance "${IPV4_REGIONS[2]}" "${IPV4_REGIONS[2]}-ipv4-bgp-tertiary-1c1g" "3" "false" || { echo "Failed to create tertiary instance"; exit 1; }
        ;;
      ipv6)
        log "Deploying IPv6-only BGP Anycast infrastructure..." "INFO"
        
        # Create IPv6 instance
        create_instance "${IPV6_REGION}" "${IPV6_REGION}-ipv6-bgp-1c1g" "1" "true" || { echo "Failed to create IPv6 instance"; exit 1; }
        ;;
      dual|*)
        ...
        create_instance "${BGP_REGIONS[3]}" "${BGP_REGIONS[3]}-dual-bgp-quaternary-1c1g" "3" "true" || { echo "Failed to create quaternary instance"; exit 1; }
        
        # All servers have dual-stack IPv4/IPv6 enabled
  ...
  generate_dual_stack_bird_config "${BGP_REGIONS[0]}-ipv4-primary" "$(cat ${BGP_REGIONS[0]}-ipv4-bgp-primary-1c1g_ipv4.txt 2>/dev/null || echo "0.0.0.0")" 0 ""
  generate_dual_stack_bird_config "${BGP_REGIONS[1]}-ipv4-secondary" "$(cat ${BGP_REGIONS[1]}-ipv4-bgp-secondary-1c1g_ipv4.txt 2>/dev/null || echo "0.0.0.0")" 1 ""
  generate_dual_stack_bird_config "${BGP_REGIONS[2]}-ipv4-tertiary" "$(cat ${BGP_REGIONS[2]}-ipv4-bgp-tertiary-1c1g_ipv4.txt 2>/dev/null || echo "0.0.0.0")" 2 ""
  generate_dual_stack_bird_config "${BGP_REGIONS[3]}-ipv4-quaternary" "$(cat ${BGP_REGIONS[3]}-ipv4-bgp-quaternary-1c1g_ipv4.txt 2>/dev/null || echo "0.0.0.0")" 2 "$(cat ${BGP_REGIONS[3]}-ipv6-bgp-1c1g_ipv6.txt 2>/dev/null || echo "::")"
  ...
  deploy_dual_stack_bird_config "${BGP_REGIONS[0]}-ipv4-primary" "$(cat ${BGP_REGIONS[0]}-ipv4-bgp-primary-1c1g_ipv4.txt 2>/dev/null || echo "")"
  deploy_dual_stack_bird_config "${BGP_REGIONS[1]}-ipv4-secondary" "$(cat ${BGP_REGIONS[1]}-ipv4-bgp-secondary-1c1g_ipv4.txt 2>/dev/null || echo "")"
  deploy_dual_stack_bird_config "${BGP_REGIONS[2]}-ipv4-tertiary" "$(cat ${BGP_REGIONS[2]}-ipv4-bgp-tertiary-1c1g_ipv4.txt 2>/dev/null || echo "")"
  deploy_dual_stack_bird_config "${BGP_REGIONS[3]}-ipv4-quaternary" "$(cat ${BGP_REGIONS[3]}-ipv4-bgp-quaternary-1c1g_ipv4.txt 2>/dev/null || echo "")"
  ...
  echo "Primary (${BGP_REGIONS[0]}): $(cat ${BGP_REGIONS[0]}-ipv4-bgp-primary-1c1g_ipv4.txt 2>/dev/null || echo "IP not found") with floating IP $(cat floating_ipv4_${BGP_REGIONS[0]}.txt 2>/dev/null || echo "IP not found")"
  echo "Secondary (${BGP_REGIONS[1]}): $(cat ${BGP_REGIONS[1]}-ipv4-bgp-secondary-1c1g_ipv4.txt 2>/dev/null || echo "IP not found") with floating IP $(cat floating_ipv4_${BGP_REGIONS[1]}.txt 2>/dev/null || echo "IP not found")"
  echo "Tertiary (${BGP_REGIONS[2]}): $(cat ${BGP_REGIONS[2]}-ipv4-bgp-tertiary-1c1g_ipv4.txt 2>/dev/null || echo "IP not found") with floating IP $(cat floating_ipv4_${BGP_REGIONS[2]}.txt 2>/dev/null || echo "IP not found")"
  echo "Quaternary (${BGP_REGIONS[3]}): $(cat ${BGP_REGIONS[3]}-ipv4-bgp-quaternary-1c1g_ipv4.txt 2>/dev/null || echo "IP not found") with floating IP $(cat floating_ipv4_${BGP_REGIONS[3]}.txt 2>/dev/null || echo "IP not found")"
  echo ""
  echo "All servers have dual-stack IPv4/IPv6 capabilities"
  echo ""
