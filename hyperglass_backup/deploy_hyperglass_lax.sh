#!/bin/bash
# Script to deploy hyperglass on LAX BGP server using direct GitHub repo
...
echo -e "${BOLD}Starting Hyperglass Deployment for LAX server${RESET}"
echo "============================================="
echo -e "${GREEN}LAX Server IP:${RESET} $LAX_IP"
...
EOF
  ...
  cat > /tmp/lax_hyperglass_setup.sh << 'EOFMARKER'
#!/bin/bash
set -e
...
# Save credentials
echo "DOMAIN=$DOMAIN" > /tmp/hyperglass_credentials.txt
echo "LETSENCRYPT_EMAIL=$LETSENCRYPT_EMAIL" >> /tmp/hyperglass_credentials.txt
...
DEBIAN_FRONTEND=noninteractive apt install -y apt-transport-https ca-certificates curl software-properties-common git socat python3 python3-pip iptables-persistent netfilter-persistent jq apache2-utils npm
...
ADMIN_PASSWORD=$(openssl rand -base64 12 2>/dev/null || echo "SecureTraefikAdmin")
HASHED_PASSWORD=$(htpasswd -nb admin "$ADMIN_PASSWORD" | sed -e s/\\$/\\$\\$/g)
echo "Generated Traefik admin password: $ADMIN_PASSWORD"
echo "TRAEFIK_ADMIN_PASSWORD=$ADMIN_PASSWORD" >> /tmp/hyperglass_credentials.txt
...
echo "Cloning Hyperglass from GitHub..."
...
# Set up hyperglass configuration
mkdir -p $CONFIG_DIR
...
# Create docker-compose.yml for hyperglass
...
# Try running hyperglass with GitHub image
cd $DEST_DIR
docker compose down
docker compose pull
docker compose up -d

# If the GitHub image doesn't work, try building it locally
if ! docker ps | grep -q hyperglass; then
  echo "GitHub image failed, trying to build locally..."
  
  # Create an alternate docker-compose.yml for local build
  cat > $DEST_DIR/docker-compose.yml << LOCALDOCKER
services:
  hyperglass:
    build: .
    container_name: hyperglass
...
if docker ps | grep -q hyperglass; then
  ...
  # Check Docker logs
  docker logs hyperglass || echo "No logs available"
fi
...
EOFMARKER
  ...
  chmod +x /tmp/lax_hyperglass_setup.sh
  
  # Copy the files to the server
  echo "Copying files to $server_ip..."
  scp -o StrictHostKeyChecking=no -i "$SSH_KEY_PATH" /tmp/lax_hyperglass_setup.sh "root@$server_ip:/tmp/lax_hyperglass_setup.sh"
  ...
  ssh -o StrictHostKeyChecking=no -i "$SSH_KEY_PATH" "root@$server_ip" "bash /tmp/lax_hyperglass_setup.sh"
  ...
  TRAEFIK_RUNNING=$(ssh -o StrictHostKeyChecking=no -i "$SSH_KEY_PATH" "root@$server_ip" "docker ps | grep -w traefik || echo 'not running'")
  
  # Try different methods of checking hyperglass status
  HYPERGLASS_RUNNING=$(ssh -o StrictHostKeyChecking=no -i "$SSH_KEY_PATH" "root@$server_ip" "(docker ps | grep -w hyperglass || systemctl is-active hyperglass 2>/dev/null) || echo 'not running'")
  
  if [[ "$TRAEFIK_RUNNING" != *"not running"* ]]; then
    echo -e "${GREEN}✓ Traefik is running successfully on $server_ip${RESET}"
  ...
  if [[ "$HYPERGLASS_RUNNING" != *"not running"* ]]; then
    echo -e "${GREEN}✓ Hyperglass is running successfully on $server_ip${RESET}"
    HYPERGLASS_SUCCESS=true
  else
    echo -e "${RED}✗ Hyperglass failed to start${RESET}"
    # Try to diagnose the issue
    echo "Checking status and logs:"
    ssh -o StrictHostKeyChecking=no -i "$SSH_KEY_PATH" "root@$server_ip" "cat /tmp/hyperglass_status.env || echo 'Status file not found'"
    HYPERGLASS_SUCCESS=false
  fi
  
  # Get domain and password information
  DOMAIN=${DOMAIN:-"infinitum-nihil.com"}
  ADMIN_PASSWORD=$(ssh -o StrictHostKeyChecking=no -i "$SSH_KEY_PATH" "root@$server_ip" "cat /tmp/hyperglass_credentials.txt | grep TRAEFIK_ADMIN_PASSWORD | cut -d= -f2" 2>/dev/null || echo "Password not found")
