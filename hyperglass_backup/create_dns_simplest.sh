#!/bin/bash
# Simplest possible script to create DNS records with DNSMadeEasy API
...
if [ -z "$API_KEY" ] || [ -z "$SECRET_KEY" ]; then
  echo "Error: API credentials are missing. Check your .env file."
  exit 1
fi
...
test_api() {
  ...
  HMAC=$(echo -n "GET${REQUEST_DATE}" | openssl dgst -sha1 -hmac "$SECRET_KEY" | cut -d ' ' -f2)
...
create_record() {
  ...
  if [ -n "$RECORD_ID" ] && [ "$RECORD_ID" != "null" ]; then
    ...
    REQUEST_DATE=$(date -u "+%a, %d %b %Y %H:%M:%S GMT")
    HMAC=$(echo -n "PUT${REQUEST_DATE}" | openssl dgst -sha1 -hmac "$SECRET_KEY" | cut -d ' ' -f2)
    ...
    REQUEST_DATE=$(date -u "+%a, %d %b %Y %H:%M:%S GMT")
    HMAC=$(echo -n "POST${REQUEST_DATE}" | openssl dgst -sha1 -hmac "$SECRET_KEY" | cut -d ' ' -f2)
