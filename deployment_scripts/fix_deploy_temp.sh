...
# Backup the current deploy_temp.sh
cp deploy_temp.sh deploy_temp.sh.bak

# Fix the script structure and remove dangling sections
...
if [ -z "\$DNS_API_SECRET" ]; then
    # Check if it is in the .env file but not loaded
...
SETUP
    ...
    sed -i "s/\${ANYCAST_IPV4}/\$ANYCAST_IPV4/g" /tmp/server_setup.sh
    sed -i "s/\${ANYCAST_IPV6}/\$ANYCAST_IPV6/g" /tmp/server_setup.sh
    
    # Copy and execute the setup script on the remote server
    scp /tmp/server_setup.sh "root@\$server_ip:/tmp/"
    ssh "root@\$server_ip" "bash /tmp/server_setup.sh"
    
    echo -e "\${GREEN}✓ Deployment to \$server_name complete\${RESET}"
}

# Deploy to each server
echo "Starting deployments..."
[ -n "\$PRIMARY_IP" ] && deploy_to_server "\$PRIMARY_IP" "Primary" "LAX"
[ -n "\$SECONDARY_IP" ] && deploy_to_server "\$SECONDARY_IP" "Secondary" "EWR"
[ -n "\$TERTIARY_IP" ] && deploy_to_server "\$TERTIARY_IP" "Tertiary" "MIA"
[ -n "\$QUATERNARY_IP" ] && deploy_to_server "\$QUATERNARY_IP" "Quaternary" "ORD"

echo -e "\${GREEN}All deployments complete!\${RESET}"
EOF

chmod +x deploy_temp.sh
echo "Fixed deploy_temp.sh and made it executable" 